...
///
/// ## Configuration
/// Objects are stored to disk. Storage location can be configured via `public init(url: URL) throws`.
/// By default the `PapyrusStore` will use a directory inside Application Support.
///
/// ## Migration
///
/// ## Saving
...
/// When an object is added to the store, it is firstly added to the in-memory cache
/// and then eventually written to the store.
///
...
///
/// `PapyrusStore` also understands relationships.
/// If we continue with our `Car` modelling...Let's imagine we have an
/// app that fetches a list of car manufacturers and their cars.
///
...
/// ```
///
/// Because `Car` also conforms to `Papyrus`, `PapyrusStore` will also persist the cars
/// when it saves the manufacturer.
///
/// ## Fetching by ID
...
///
/// When fetching a collection of objects a Combine publisher is returned
/// which will emit the collection of objects. Unless specified the publisher will
/// continue to emit a collection objects whenever a change is detected.
///
